...
@section('title', __('Gyms across Uzbekistan'))

@section('content')
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14 space-y-10">
        <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
                <p class="text-sm uppercase tracking-[0.4em] text-gray-500">{{ __('Boxes & studios') }}</p>
                <h1 class="text-3xl font-bold mt-2">{{ __('Find your training ground') }}</h1>
                <p class="text-gray-400 max-w-2xl">{{ __('Filter by city or type to locate the perfect CrossFit box or functional training facility.') }}</p>
            </div>
            <form method="GET" class="bg-slate-900/70 border border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row gap-3 text-sm">
                <div class="flex flex-col">
                    <label for="city" class="text-gray-400 mb-1">{{ __('City') }}</label>
                ...
                <div class="flex flex-col">
                    <label for="type" class="text-gray-400 mb-1">{{ __('Type') }}</label>
        ...
        <div class="grid gap-6 md:grid-cols-2">
            @forelse($gyms as $gym)
                <a href="{{ route('gyms.show', $gym) }}" class="bg-slate-900/70 border border-slate-800 rounded-3xl p-6 hover:border-orange-500/70 transition flex flex-col gap-4 min-h-[320px] break-words">
                    ...
                    <div class="grid grid-cols-3 gap-4 text-sm text-gray-400">
                        <div>
                            <p class="text-gray-500 uppercase tracking-wide text-xs">{{ __('Coaches') }}</p>
                            <p class="text-white text-xl font-semibold">{{ $gym->coaches_count }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 uppercase tracking-wide text-xs">{{ __('Contact') }}</p>
                            <p class="break-all">{{ $gym->phone ?? '—' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 uppercase tracking-wide text-xs">{{ __('Email') }}</p>
            ...
            @empty
                <div class="col-span-full text-center py-16 bg-slate-900/50 border border-dashed border-slate-700 rounded-3xl">
                    <p class="text-gray-400">{{ __('No gyms match your filters yet.') }}</p>
                </div>
            @endforelse
        </div>
