...
@section('title', __('text.Gyms map'))

@section('content')
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
        <div class="flex flex-col gap-4">
            <div>
                <p class="text-sm uppercase tracking-wide text-orange-400 font-semibold">{{ __('text.Gyms') }}</p>
                <h1 class="text-3xl sm:text-4xl font-bold text-white">{{ __('text.Gyms map') }}</h1>
                <p class="text-gray-400 mt-2 max-w-2xl">
                    {{ __('text.Explore every box on the map and drop a pin to find the closest pickup location.') }}
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-3 text-sm text-gray-300">
                <span class="px-3 py-1 rounded-full border border-slate-700">
                    {{ __('text.:count gyms pinned', ['count' => $gyms->count()]) }}
                </span>
...
@push('scripts')
    ...
    <script>
        ...
        const websiteLabel = @json(__('text.Website'));
            ...
            gyms.forEach((gym) => {
                ...
                L.marker(coords).addTo(map).bindPopup(`
                    <div class="space-y-1">
                        ...
                        ${gym.website ? `<a href="${gym.website}" target="_blank" class="text-xs text-orange-500 underline">${websiteLabel}</a>` : ''}
