...
@section('content')
    ...
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
        <div class="flex flex-col gap-2">
            <p class="text-sm uppercase tracking-[0.4em] text-gray-500">{{ __('text.Athlete dashboard') }}</p>
            <h1 class="text-3xl font-bold">{{ $user->name }}</h1>
            <p class="text-gray-400">{{ __('text.Overview of recent WODs, RX history, and personal bests.') }}</p>
        </div>

        <div class="grid gap-4 md:grid-cols-4">
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-5">
                <p class="text-xs uppercase tracking-[0.3em] text-gray-500">{{ __('text.Total WODs') }}</p>
                <p class="text-4xl font-black mt-4">{{ $stats['total_wods'] }}</p>
            </div>
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-5">
                <p class="text-xs uppercase tracking-[0.3em] text-gray-500">{{ __('text.RX Rate') }}</p>
                <p class="text-4xl font-black mt-4 text-orange-400">{{ $stats['rx_rate'] }}%</p>
            </div>
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-5">
                <p class="text-xs uppercase tracking-[0.3em] text-gray-500">{{ __('text.Best time') }}</p>
                <p class="text-4xl font-black mt-4">{{ $formatTime($stats['best_time_seconds'] ?? null) }}</p>
            </div>
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-5">
                <p class="text-xs uppercase tracking-[0.3em] text-gray-500">{{ __('text.Heaviest lift') }}</p>
                <p class="text-4xl font-black mt-4">{{ $stats['heaviest_lift'] ? $stats['heaviest_lift'] . __('text.kg') : '—' }}</p>
            </div>
        </div>
        ...
        <section class="grid gap-6 lg:grid-cols-2">
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-semibold">{{ __('text.Recent WODs') }}</h2>
                ...
                <div class="space-y-4">
                    ...
                    @empty
                        <p class="text-gray-400">{{ __('text.No logged workouts yet.') }}</p>
                    @endforelse
                </div>
            </div>
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-6">
                <h2 class="text-xl font-semibold mb-4">{{ __('text.Personal bests') }}</h2>
                <div class="space-y-4">
                            ...
                            <div class="mt-3 grid grid-cols-3 text-sm">
                                ...
                                <div>
                                    ...
                                    <p class="text-xl font-bold">
                                        {{ $best->is_rx ? $scaleLabels['rx'] : \Illuminate\Support\Str::upper($scaleLabels[$best->result_scale] ?? $best->result_scale) }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">{{ __('text.Weight') }}</p>
                                    <p class="text-xl font-bold">{{ $best->weight_in_kg ? $best->weight_in_kg . __('text.kg') : __('text.Bodyweight') }}</p>
                                </div>
                            </div>
                        </div>
                    @empty
                        <p class="text-gray-400">{{ __('text.Log workouts to see your PRs.') }}</p>
                    @endforelse
                </div>
            </div>
        </section>
        ...
        <section class="bg-slate-900/40 border border-slate-800 rounded-3xl overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
                <h2 class="text-xl font-semibold">{{ __('text.History') }}</h2>
                <p class="text-sm text-gray-500">{{ __('text.Full log of your submissions') }}</p>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-800 text-sm">
                    <thead class="bg-slate-900/80 text-gray-400 uppercase tracking-wide text-xs">
                        <tr>
                            <th class="px-4 py-3 text-left">{{ __('text.Date') }}</th>
                            <th class="px-4 py-3 text-left">{{ __('text.WOD') }}</th>
                            <th class="px-4 py-3 text-left">{{ __('text.Type') }}</th>
                            <th class="px-4 py-3 text-left">{{ __('text.Score') }}</th>
                            <th class="px-4 py-3 text-left">{{ __('text.RX?') }}</th>
                            <th class="px-4 py-3 text-left">{{ __('text.Notes') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-900">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-gray-400">{{ __('text.No history yet.') }}</td>
                            </tr>
                    @endforelse
                    </tbody>
                </table>
