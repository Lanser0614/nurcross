...
@section('title', __('CrossFit Uzbekistan — Community Hub'))
...
@section('content')
    <section class="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-950 to-black">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 grid gap-10 lg:grid-cols-2 items-center">
            <div>
                <p class="uppercase tracking-[0.4em] text-xs text-gray-400">{{ __('Stronger Together') }}</p>
                ...
                <p class="text-lg text-gray-300 mt-6 max-w-xl">
                    {{ __('Discover local boxes, track brutal WODs, and master iconic movements. Built for the athletes pushing the CrossFit scene across Uzbekistan.') }}
                </p>
            ...
            <div class="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 shadow-2xl shadow-orange-500/10">
                <div class="flex flex-wrap items-center justify-between text-gray-400 text-[0.55rem] sm:text-[0.65rem] lg:text-xs uppercase tracking-[0.2em] sm:tracking-[0.35em] gap-x-4 gap-y-1">
                    <span class="leading-tight text-left shrink">{{ __('Today\'s intensity') }}</span>
                    <span class="leading-tight text-right shrink">{{ __('RX focus') }}</span>
                </div>
                <div class="mt-6 text-center">
                    <p class="text-sm text-gray-400">{{ __('Benchmark highlight') }}</p>
...
{{--                            <p class="text-gray-400">{{ __('Time cap') }}</p>--}}
{{--                            <p class="text-2xl font-bold text-white">15:00</p>--}}
{{--                        </div>--}}
{{--                        <div class="bg-slate-950/60 p-4 rounded-xl border border-slate-800 text-center sm:text-left">--}}
{{--                            <p class="text-gray-400">{{ __('Difficulty') }}</p>--}}
{{--                            <p class="text-2xl font-bold text-white wrap-break-word">{{ __('Advanced') }}</p>--}}
{{--                        </div>--}}

{{--                        <div class="bg-slate-950/60 p-4 rounded-xl border border-slate-800 text-center sm:text-left">--}}
{{--                            <p class="text-gray-400">{{ __('Logged today') }}</p>--}}
    ...
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14 space-y-16">
        <div>
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-2xl font-bold">{{ __('Popular gyms') }}</h3>
                <a href="{{ route('gyms.index') }}" class="text-sm text-orange-400 hover:underline">{{ __('View all') }}</a>
            </div>
                ...
                @foreach($popularGyms as $gym)
                    <a href="{{ route('gyms.show', $gym) }}" class="bg-slate-900/70 border border-slate-800 rounded-2xl p-5 hover:border-orange-500/70 transition flex flex-col gap-4">
                        ...
                        <div class="flex gap-6 text-sm text-gray-400">
                            <div>
                                <p class="text-gray-500">{{ __('Coaches') }}</p>
                                <p class="text-white font-semibold">{{ $gym->coaches_count }}</p>
                            </div>
                            <div>
                                <p class="text-gray-500">{{ __('Telephone') }}</p>
        ...
        <div>
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-2xl font-bold">{{ __('Featured WODs') }}</h3>
                <a href="{{ route('wods.index') }}" class="text-sm text-orange-400 hover:underline">{{ __('View all') }}</a>
            </div>
            <div class="grid gap-6 md:grid-cols-2">
        ...
        <div>
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-2xl font-bold">{{ __('New movements added') }}</h3>
                <a href="{{ route('movements.index') }}" class="text-sm text-orange-400 hover:underline">{{ __('Browse all') }}</a>
            </div>
