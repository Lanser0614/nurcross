...
@section('title', __('text.Movement Library'))

@section('content')
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14 space-y-10">
        <div class="flex flex-col gap-4">
            <p class="text-sm uppercase tracking-[0.4em] text-gray-500">{{ __('text.Skill library') }}</p>
            <h1 class="text-3xl font-bold">{{ __('text.Master every movement') }}</h1>
            <p class="text-gray-400 max-w-3xl">
                {{ __('text.Search by category, difficulty, or equipment to review standards, technique notes, and demo videos.') }}
            </p>
        </div>

        <form method="GET" class="grid gap-4 md:grid-cols-3 bg-slate-900/60 border border-slate-800 rounded-3xl p-6 text-sm">
            <div>
                <label class="text-gray-400 block mb-2" for="category">{{ __('text.Category') }}</label>
            ...
            <div>
                <label class="text-gray-400 block mb-2" for="equipment">{{ __('text.Equipment') }}</label>
            ...
            <div class="md:col-span-3 flex justify-end">
                <button class="px-5 py-3 rounded-2xl bg-orange-500 text-black font-semibold shadow shadow-orange-500/30">{{ __('text.Apply filters') }}</button>
            </div>
        </form>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            @forelse($movements as $movement)
                <a href="{{ route('movements.show', $movement) }}" class="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 hover:border-orange-500/60 transition flex flex-col gap-3">
                    ...
                    <p class="text-xs text-gray-500">{{ __('text.Equipment: :equipment', ['equipment' => $movement->equipment ?? __('text.Bodyweight')]) }}</p>
                </a>
            @empty
                <p class="text-gray-400">{{ __('text.No movements match your filters.') }}</p>
            @endforelse
        </div>
