...
@section('content')
    <div class="max-w-md mx-auto px-4 py-16">
        <div class="bg-slate-900/70 border border-slate-800 rounded-2xl shadow-2xl shadow-orange-500/10 p-8">
            <h1 class="text-2xl font-bold text-orange-400 mb-6">{{ __('Create your account') }}</h1>
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                @csrf

                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-300 mb-2">{{ __('Name') }}</label>
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-gray-300 mb-2">{{ __('Confirm password') }}</label>
            ...
            <p class="mt-6 text-sm text-gray-400 text-center">
                ...
                <a href="{{ route('login') }}" class="text-orange-400 font-semibold hover:underline">{{ __('Sign in') }}</a>
            </p>
        </div>
    </div>
@endsection
